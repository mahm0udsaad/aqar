      ...
      <footer className="bg-foreground text-primary-foreground py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div>
              <div
                className={`flex items-center space-x-2 mb-4 ${lng === "ar" ? "flex-row-reverse space-x-reverse" : ""}`}
              >
                <div className="w-8 h-8 bg-primary rounded-lg flex items-center justify-center">
                  <Building className="h-5 w-5 text-primary-foreground" />
                </div>
                <span className="font-bold text-xl">Aqar</span>
              </div>
              <p className="text-primary-foreground/80 mb-4">{dict.footer.companyDesc}</p>
            </div>
            ...
            <div>
              ...
              <ul className="space-y-2">
                <li>
                  <Link
                    href={`/${lng}/search?type=sale`}
                    className="text-primary-foreground/80 hover:text-white transition-colors"
                  >
                    {dict.footer.buyProperty}
                  </Link>
                </li>
                <li>
                  <Link
                    href={`/${lng}/search?type=rent`}
                    className="text-primary-foreground/80 hover:text-white transition-colors"
                  >
                    {dict.footer.rentProperty}
                  </Link>
                </li>
                <li>
                  <Link
            ...
            {/* Popular Areas */}
            <div>
              <h3 className="font-semibold text-lg mb-4">{dict.footer.popularAreas}</h3>
              <ul className="space-y-2">
                {popularAreas.slice(0, 4).map((area, index) => (
                  <li key={index}>
                    <Link
                      href={`/${lng}/search?area=${encodeURIComponent(area.name)}`}
                      className="text-primary-foreground/80 hover:text-white transition-colors"
                    >
                      {area.name}
                    </Link>
            ...
            {/* Contact */}
            <div>
              <h3 className="font-semibold text-lg mb-4">{dict.footer.contactUs}</h3>
              <div className="space-y-2 text-primary-foreground/80">
                <p>📞 {dict.footer.contactPhone}</p>
                <p>✉️ {dict.footer.contactEmail}</p>
                <p>📍 {dict.footer.contactAddress}</p>
              </div>
            </div>
          </div>
          ...
          <div className="border-t border-gray-800 mt-8 pt-8 text-center">
            <p className="text-primary-foreground/60">
              © 2024 Aqarmap. {dict.footer.allRightsReserved} | {dict.footer.privacyPolicy} |{" "}
